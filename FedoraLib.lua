...
local function newStroke(parent, color, transparency, thickness, applyMode)
...
function Library:ShowPopup(titleInput, bodyInput)
    ...
    PopupFrame.Parent = ScreenGui
        ...
        local PopupCorner = newCorner(PopupFrame, UDim.new(0, 10))

        local PopupStroke = newStroke(PopupFrame, Color3.fromRGB(255, 255, 255), 0.85, 1.5, Enum.ApplyStrokeMode.Border)
...
function Library:SendNotification(titleInput, textInput)
    ...
    notifFrame.Parent = NotificationContainer
        ...
        local notifCorner = newCorner(notifFrame, UDim.new(0, 10))

        local notifStroke = newStroke(notifFrame, Color3.fromRGB(255, 255, 255), 0.85, 1)
...
function Library:CreateWindow(config)
    ...
    ToggleButton.Position = UDim2.new(0.5, -30, 0, -50)
    ...
    ToggleButton.Parent = ScreenGui
        ...
        local ToggleCorner = newCorner(ToggleButton, UDim.new(0, 6))

        local ToggleStroke = newStroke(ToggleButton, Color3.fromRGB(255, 255, 255), 0.8, 1.5, Enum.ApplyStrokeMode.Border)
    ...
    MainFrame.Parent = ScreenGui
        ...
        local MainCorner = newCorner(MainFrame, UDim.new(0, 16))

        local MainStroke = newStroke(MainFrame, Color3.fromRGB(255, 255, 255), 0.85, 1.5, Enum.ApplyStrokeMode.Border)
    ...
    ContentContainer.Size = UDim2.new(1, -36, 1, -50)
    ContentContainer.Position = UDim2.new(0, 18, 0, 43)
    ...
    TabPill.AnchorPoint = Vector2.new(0.5, 0)
    TabPill.Size = UDim2.new(0, 0, 0, 37)
    TabPill.Position = UDim2.new(0.5, 0, 0, -19)
    TabPill.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    TabPill.ClipsDescendants = true
    TabPill.Parent = ScreenGui

        local TabPillCorner = newCorner(TabPill, UDim.new(1, 0))

        local TabPillStroke = newStroke(TabPill, Color3.fromRGB(255, 255, 255), 0.85, 1.5, Enum.ApplyStrokeMode.Border)

    local TabPillHeight = 37
    ...
    function Window:CreateTab(tabNameInput)
        ...
        local pillWidth = TabButtonWidth * totalTabsPlanned
        TabPill.Size = UDim2.new(0, pillWidth, 0, TabPillHeight)

        for i, existingBtn in ipairs(tabButtons) do
            existingBtn.Size = UDim2.new(0, TabButtonWidth - 4, 1, -6)
            existingBtn.Position = UDim2.new(0, (i - 1) * TabButtonWidth + 2, 0, 3)
        end

        local btn = Instance.new("TextButton")
        btn.Size = UDim2.new(0, TabButtonWidth - 4, 1, -6)
        btn.Position = UDim2.new(0, (index - 1) * TabButtonWidth + 2, 0, 3)
        btn.BackgroundTransparency = 1
        btn.TextColor3 = (index == 1) and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(130, 130, 130)
        btn.Font = Enum.Font.GothamBold
        btn.AutoButtonColor = false
        btn.Parent = TabPill
        registerText(btn, "Text", tabNameInput, 12)

        local btnCorner = Instance.new("UICorner")
        local btnHighlight = Instance.new("Frame")
        btnHighlight.Size = UDim2.new(0, TabButtonWidth - 20, 1, -16)
        btnHighlight.Position = UDim2.new(0, (index - 1) * TabButtonWidth + 10, 0, 8)
        btnHighlight.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
        btnHighlight.BackgroundTransparency = (index == 1) and 0 or 1
        btnHighlight.ZIndex = 1
        btnHighlight.Parent = TabPill

        btn.ZIndex = 2

                local btnHighlightCorner = newCorner(btnHighlight, UDim.new(0, 6))

        btnCorner.CornerRadius = UDim.new(0, 8)
        btnCorner.Parent = btn

        local underline = Instance.new("Frame")
        underline.Size = UDim2.new(0, 28, 0, 3)
        underline.AnchorPoint = Vector2.new(0.5, 0)
        underline.Position = UDim2.new(0.5, 0, 1, -4.5)
        underline.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        underline.BackgroundTransparency = 1
        underline.Parent = btn

                local underlineCorner = newCorner(underline, UDim.new(1, 0))

        table.insert(tabButtons, btn)
        table.insert(tabUnderlines, underline)
        table.insert(tabHighlights, btnHighlight)
        ...
        btn.MouseButton1Click:Connect(function()
            for idx, v in ipairs(tabViews) do
                ...
                if idx == index then
                    ...
                    TweenService:Create(tabHighlights[idx], TweenInfo.new(0.15), { BackgroundTransparency = 0 }):Play()
                    TweenService:Create(tabUnderlines[idx], TweenInfo.new(0.15), { BackgroundTransparency = 0 }):Play()
                else
                    tabButtons[idx].TextColor3 = Color3.fromRGB(130, 130, 130)
                    TweenService:Create(tabHighlights[idx], TweenInfo.new(0.15), { BackgroundTransparency = 1 }):Play()
                    TweenService:Create(tabUnderlines[idx], TweenInfo.new(0.15), { BackgroundTransparency = 1 }):Play()
                end
            end
        end)
        ...
        function Tab:CreateToggle(toggleConfig)
            ...
            addElement(ToggleContainer, 42)
                        ...
                        local TogCorner = newCorner(ToggleContainer, UDim.new(0, 8))

                        local TogStroke = newStroke(ToggleContainer, Color3.fromRGB(255, 255, 255), 0.9, 1)
            ...
            SwitchTrack.Parent = ToggleContainer
                        ...
                        local TrackCorner = newCorner(SwitchTrack, UDim.new(1, 0))
            ...
            SwitchKnob.Parent = SwitchTrack
                        ...
                        local KnobCorner = newCorner(SwitchKnob, UDim.new(1, 0))
        ...
        function Tab:CreateDropdown(dropdownConfig)
            ...
            local elementEntry = addElement(DropdownContainer, 38)
                        ...
                        local DropCorner = newCorner(DropdownContainer, UDim.new(0, 8))

                        local DropStroke = newStroke(DropdownContainer, Color3.fromRGB(255, 255, 255), 0.9, 1)
            ...
            local DropdownBtnPadding = Instance.new("UIPadding")
            DropdownBtnPadding.PaddingLeft = UDim.new(0, 16)
            DropdownBtnPadding.Parent = DropdownBtn
            ...
            local function buildOptions()
                ...
                for _, optInput in ipairs(options) do
                    ...
                    optBtn.TextSize = fontSizeFor(11)
                                        ...
                                        local optCorner = newCorner(optBtn, UDim.new(0, 6))
        ...
        function Tab:CreateSlider(sliderConfig)
            ...
            addElement(SliderContainer, 54)
                        ...
                        local SliderCorner = newCorner(SliderContainer, UDim.new(0, 8))

                        local SliderStroke = newStroke(SliderContainer, Color3.fromRGB(255, 255, 255), 0.9, 1)
            ...
            ValueBubble.Parent = SliderContainer
                        ...
                        local BubbleCorner = newCorner(ValueBubble, UDim.new(0, 6))

                        local BubbleStroke = newStroke(ValueBubble, Color3.fromRGB(255, 255, 255), 0.88, 1)
            ...
            Track.Parent = HitArea
                        ...
                        local TrackCorner = newCorner(Track, UDim.new(1, 0))
            ...
            Fill.Parent = Track
                        ...
                        local FillCorner = newCorner(Fill, UDim.new(1, 0))
            ...
            Knob.Parent = Track
                        ...
                        local KnobCorner = newCorner(Knob, UDim.new(1, 0))

                        local KnobStroke = newStroke(Knob, Color3.fromRGB(0, 0, 0), 0.7, 1)
        ...
        function Tab:CreateButton(buttonConfig)
            ...
            addElement(Btn, 38)
                        ...
                        local BtnCorner = newCorner(Btn, UDim.new(0, 8))

                        local BtnStroke = newStroke(Btn, Color3.fromRGB(255, 255, 255), 0.9, 1)
        ...
        function Tab:CreateTextbox(textboxConfig)
            ...
            local flagName = textboxConfig.Flag or textboxConfig.Name or ("Textbox" .. tostring(math.random(100000, 999999)))
            local nameInput = textboxConfig.Name or {EN = "Textbox", AR = "مربع نص"}
            local placeholderInput = textboxConfig.Placeholder or {EN = "", AR = ""}
            local defaultVal = textboxConfig.Default or ""
            local callback = textboxConfig.Callback or function() end

            Library.Flags[flagName] = defaultVal

            local TextboxContainer = Instance.new("Frame")
            TextboxContainer.Size = UDim2.new(1, 0, 0, 38)
            TextboxContainer.BackgroundColor3 = Color3.fromRGB(12, 12, 12)
            TextboxContainer.Parent = view

            addElement(TextboxContainer, 38)

                        local TextboxCorner = newCorner(TextboxContainer, UDim.new(0, 8))

                        local TextboxStroke = newStroke(TextboxContainer, Color3.fromRGB(255, 255, 255), 0.9, 1)

            local TextboxLabel = Instance.new("TextLabel")
            TextboxLabel.Size = UDim2.new(1, -152, 1, 0)
            ...
            registerText(TextboxLabel, "Text", nameInput, 12)

            local InputArea = Instance.new("Frame")
            InputArea.Size = UDim2.new(0, 130, 0, 26)
            InputArea.Position = UDim2.new(1, -142, 0.5, -13)
            InputArea.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
            InputArea.Parent = TextboxContainer

                        local InputCorner = newCorner(InputArea, UDim.new(0, 6))

                        local InputStroke = newStroke(InputArea, Color3.fromRGB(255, 255, 255), 0.85, 1)
            ...
            InputBox.TextSize = fontSizeFor(12)
            InputBox.TextXAlignment = Enum.TextXAlignment.Left
            InputBox.ClearTextOnFocus = false
            InputBox.Parent = InputArea

            registerCustomText(function()
                InputBox.PlaceholderText = resolveText(placeholderInput)
                InputBox.TextSize = fontSizeFor(12)
            end)
...
local Window = Library:CreateWindow({
    Title = {EN = "My Game Hub", AR = "لوحة اللعبة"}
})

local MainTab = Window:CreateTab({EN = "Main", AR = "الرئيسية"})

MainTab:CreateLabel({ Text = {EN = "Welcome to the hub", AR = "مرحبا بك"} })

MainTab:CreateButton({
    Name = {EN = "Teleport to Lobby", AR = "انتقل إلى الردهة"},
    Callback = function()
...
MainTab:CreateToggle({
    Name = {EN = "FPS Boost (Low Graphics)", AR = "تحسين الأداء"},
    Flag = "FPSBoost",
    Default = false,
    Callback = function(enabled)
        print("FPS Boost toggled:", enabled)
    end
})
...
MainTab:CreateDropdown({
    ...
    OptionLangCodes = { ["English"] = "EN", ["العربية"] = "AR" },
    Callback = function(selected)
        print("Language switched to", selected)
    end
})

local ShopTab = Window:CreateTab({EN = "Pet Shop", AR = "متجر الحيوانات"})

ShopTab:CreateLabel({ Text = {EN = "Buy pets with in-game coins", AR = "اشترِ حيوانات"} })

local pets = {
    {Name = "Dragon", Price = 500},
    {Name = "Phoenix", Price = 750},
    {Name = "Turtle", Price = 100},
}

for _, pet in ipairs(pets) do
    ShopTab:CreateButton({
        Name = {EN = ("Buy %s - %d coins"):format(pet.Name, pet.Price)},
        Callback = function()
            print("Would buy:", pet.Name, "for", pet.Price)
        end
    })
end

ShopTab:CreateDropdown({
    Name = {EN = "Equip Pet", AR = "تجهيز حيوان"},
    Flag = "EquippedPet",
    Options = {"Dragon", "Phoenix", "Turtle", "None"},
    Default = "None",
    Callback = function(selected)
        print("Would equip:", selected)
    end
})
...
SettingsTab:CreateButton({
    Name = {EN = "Show Info Popup", AR = "عرض معلومات"},
    Callback = function()
        Library:ShowPopup(
            {EN = "About", AR = "حول"},
            {EN = "This is a demo popup for testing the library.", AR = "هذه نافذة تجريبية"}
        )
    end
})
