...
local StarCanvas = Instance.new("Frame")
    ...
    local STAR_COUNT = 45
    local CONNECT_DISTANCE = 70
    local STAR_SPEED_MIN = 4
    local STAR_SPEED_MAX = 10

    local stars = {}
    local linePool = {}
    local linePoolIndex = 1

    local function createStar()
        local angle = math.rad(math.random(0, 360))
        local speed = math.random(STAR_SPEED_MIN, STAR_SPEED_MAX)
    ...
    local function getLineFromPool()
        if linePool[linePoolIndex] then
            local line = linePool[linePoolIndex]
    ...
    local function drawLine(p1, p2, opacity)
        ...
        line.Rotation = angle
        line.BackgroundTransparency = 1 - (opacity * 0.25)
    end
    ...
    RunService.RenderStepped:Connect(function(dt)
        ...
        for _, star in ipairs(stars) do
            ...
            if star.x < 0 or star.x > boundsX then star.vx = -star.vx end
            if star.y < 0 or star.y > boundsY then star.vy = -star.vy end

            star.guiElement.Position = UDim2.new(0, star.x, 0, star.y)
        ...
        for i = 1, visibleCount do
            for j = i + 1, visibleCount do
                local s1 = visibleStars[i]
                local s2 = visibleStars[j]

                local dx = s1.x - s2.x
                local dy = s1.y - s2.y
                local dist = math.sqrt(dx * dx + dy * dy)

                if dist < CONNECT_DISTANCE then
                    local opacity = (1 - (dist / CONNECT_DISTANCE))
                    drawLine(Vector2.new(s1.x, s1.y), Vector2.new(s2.x, s2.y), opacity)
                end
            end
        end
    end)
